...
Public Class F_Project
    ...
    Public Sub Project_Bind(ByVal oForm As SAPbouiCOM.Form)
        Try
            oForm.DataSources.DataTables.Add("PVLO")
            oForm.DataSources.DataTables.Add("DOLO") 'BILO
            oForm.DataSources.DataTables.Add("BILO") 'BILO
            oForm.DataSources.DataTables.Add("PVPR")
            oForm.PaneLevel = 1
            DocNumber_Local()
            ...
            CFL_SalesOrder_International(oForm, SBO_Application, "PR")
    ...
    Private Sub ComboLoad_Division(ByRef Oform As SAPbouiCOM.Form, ByRef oCombo As SAPbouiCOM.ComboBox)
        Try
            ...
            Do While Not oRecordSet.EoF
                ...
                oRecordSet.MoveNext()
            Loop
            oCombo.ValidValues.Remove("PR", SAPbouiCOM.BoSearchKey.psk_ByValue)
            oRecordSet = Nothing
            GC.Collect()
        Catch ex As Exception
    ...
    Public Shared Sub LoadGrid_BI(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oEdit = oForm.Items.Item("SIJ16").Specific
            Dim JobNo As String = oEdit.String
            Dim str As String = "SELECT DocEntry 'DocNum','DraftInvoice' DocumentType ,T0.[DocDate] 'BIDate', T0.[CardCode] 'Customer Code', T0.[CardName] 'Customer Name', T0.[NumAtCard] 'Ref.No', T0.[DocTotal] 'Document Total' FROM ODRF T0 WHERE T0.[ObjType] =13 and  T0.[DocStatus] ='O' and  T0.[U_AB_JobNo] = '" & JobNo & "' union all SELECT DocEntry 'DocNum','Invoice' DocumentType , T0.[DocDate] 'BIDate', T0.[CardCode] 'Customer Code', T0.[CardName] 'Customer Name', T0.[NumAtCard] 'Ref.No', T0.[DocTotal] 'Document Total' FROM OINV T0 WHERE   T0.[U_AB_JobNo] ='" & JobNo & "'"
    ...
    Public Shared Sub LoadGrid_PV(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oForm.DataSources.DataTables.Item("PVPR").ExecuteQuery(str)
            oGrid.DataTable = oForm.DataSources.DataTables.Item("PVPR")
    ...
    Public Sub DocNumber_Local()
        Try
            ...
            oRecordSet2.DoQuery("SELECT (count(*)+1) as CountNo FROM [dbo].[@AB_PRO_HEADER]  T0 WHERE T0.[CreateDate]  between '" & fdt & "' and '" & tdt & "'")
            Dim DocNum As Integer = oRecordSet2.Fields.Item(0).Value
            oEdit = oForm.Items.Item("SIJ16").Specific
            Dim DocNumLen As Integer
            DocNumLen = DocNum.ToString.Length
            If DocNum = 0 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & "00001"
            ElseIf DocNumLen = 1 And DocNum <> 0 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & "0000" & DocNum
            ElseIf DocNumLen = 2 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & "000" & DocNum
            ElseIf DocNumLen = 3 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & "00" & DocNum
            ElseIf DocNumLen = 4 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & "0" & DocNum
            ElseIf DocNumLen >= 5 Then
                oEdit.String = "PR" & Format(Now.Date, "yy") & "J" & DocNum
            End If
    ...
    Public Sub LoadJobOrder(ByVal SQNO As String)
        Try
            ...
            oRecordSet1.DoQuery("SELECT T0.[Comments],T0.DocNum,T0.[CardCode], T0.[CardName], T1.[Name],  T0.[U_AB_OrginCode], T0.[U_AB_OriginName], T0.[U_AB_OrginCodeC], T0.[U_AB_OriginNameC], T0.[U_AB_DestCode], T0.[U_AB_DestName], T0.[U_AB_DestCodeC], T0.[U_AB_DestNameC],T0.[U_AB_Desc],T0.[U_AB_TotPkg], T0.[U_AB_TotWT], T0.[U_AB_TotVol],T0.[U_AB_Divsion],T0.[U_AB_VessCode], T0.[U_AB_VessName] FROM ORDR T0 Left JOIN OCPR T1 ON T0.CntctCode = T1.CntctCode WHERE  T0.[U_AB_Divsion1] ='PR' and isnull( T0.[U_AB_Status1] ,'')='Open' and T0.DocNum='" & SQNO & "'")
            ...
            oEdit = oForm.Items.Item("ijIJ157").Specific
            ...
            oEdit = oForm.Items.Item("ijIJ100001").Specific
            ...
            oEdit.String = oRecordSet1.Fields.Item("Comments").Value
            ...
            Try
                oCombo = oForm.Items.Item("1000002").Specific
                oCombo.Select(oRecordSet1.Fields.Item("U_AB_Divsion").Value, SAPbouiCOM.BoSearchKey.psk_ByValue)
                'T0.[U_AB_VessCode], T0.[U_AB_VessName]
                oEdit = oForm.Items.Item("SIJ27").Specific
                oEdit.String = oRecordSet1.Fields.Item("U_AB_VessCode").Value
    ...
    Private Sub SBO_Application_FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean) Handles SBO_Application.FormDataEvent
        Try
            If BusinessObjectInfo.FormUID = "PRO_JOB" Then
    ...
    Private Sub SBO_Application_ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean) Handles SBO_Application.ItemEvent
        Try
            ...
            If ((pVal.FormUID = "PRO_JOB" And pVal.ItemUID = "ATTMAT" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_OK_MODE) And pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.Before_Action = False)) Then
                ...
                Next
            End If
            If ((pVal.FormUID = "PRO_JOB" And pVal.ItemUID = "1000006" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_OK_MODE) And pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.Before_Action = False)) Then
                oMatrix1 = oForm.Items.Item("ATTMAT").Specific
                oColumns = oMatrix1.Columns
                Dim i As Integer
                For i = 1 To oMatrix1.RowCount
                ...
                End If
            End If
            If ((pVal.FormUID = "PRO_JOB" And pVal.ItemUID = "155" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_OK_MODE) And pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.Before_Action = False)) Then
                oMatrix1 = oForm.Items.Item("ATTMAT").Specific
                oColumns = oMatrix1.Columns
                Dim i As Integer
                For i = 1 To oMatrix1.RowCount
                ...
                oItem.Enabled = False
            End If
            If ((pVal.FormUID = "PRO_JOB" And pVal.ItemUID = "1000005" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_OK_MODE) And pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.Before_Action = False)) Then
                Try
                ...
                If pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED Then
                    If pVal.Before_Action = False And pVal.InnerEvent = False Then
                        ...
                        ElseIf pVal.ItemUID = "ATTACH" Then
                            oForm.PaneLevel = 5
                        ElseIf pVal.ItemUID = "BI" Then
                            oForm.PaneLevel = 6
                        ElseIf pVal.ItemUID = "99" Then
                            oForm.PaneLevel = 7
                        ...
                        End If
                    End If
                    If pVal.Before_Action = True And pVal.InnerEvent = False Then
                        If pVal.ItemUID = "1" Then
                            If (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                                Try
                                    ...
                                    oEdit = oForm.Items.Item("SIJ8").Specific
                                    If oEdit.String = "" Then
                                        SBO_Application.StatusBar.SetText("Customer Name can't be Empty", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                        BubbleEvent = False
                                        Exit Sub
                                    End If
                                    '-----------End Validation------------
                                    ...
                                    '------------------------
                                    Dim JobNo As String = ""
                                    Dim NewDivision As String = ""
                                    Dim Division As String = ""
                                    Dim DocNum As String = ""
                                    oCombo = oForm.Items.Item("1000002").Specific
                                    NewDivision = oCombo.Selected.Value
                                    oEdit = oForm.Items.Item("SIJ16").Specific
                                    JobNo = oEdit.String
                                    oEdit = oForm.Items.Item("SIJ4").Specific
                                    DocNum = oEdit.String
                                    oRecordSet1 = Ocompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                    oRecordSet1.DoQuery("SELECT T0.[U_TransTo], T0.[U_TranSt], T0.[U_RefJob] FROM [dbo].[@AB_PRO_HEADER]  T0 WHERE T0.[U_JobNo] ='" & JobNo & "'")
                                    Division = oRecordSet1.Fields.Item("U_TransTo").Value
                                    If NewDivision <> "None" And oRecordSet1.RecordCount > 0 Then
                                        If NewDivision <> Division Then
                                            oRecordSet1.DoQuery("Update ORDR set U_AB_Status='Open',U_AB_Divsion='" & NewDivision & "' where DocNum='" & DocNum & "'")
                                        End If
                                    ElseIf NewDivision = "None" Then
                                        oRecordSet1.DoQuery("Update ORDR set U_AB_Status='Closed' where DocNum='" & DocNum & "'")
                                    End If
                                    '----------Document No Loading---------
                                    If pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
                                        DocNumber_Local()
...
10:                                             If System.IO.File.Exists(destPath & FileName & FileExten) Then
                ...
                ElseIf pVal.EventType = SAPbouiCOM.BoEventTypes.et_COMBO_SELECT Then
                    If pVal.Before_Action = False And pVal.InnerEvent = False And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                        If pVal.ItemUID = "1000002" Then
                            Try
                                oCombo = oForm.Items.Item("1000002").Specific
                                If oCombo.Selected.Value = "None" Then
                                    oItem = oForm.Items.Item("1000008")
                                    oItem.Enabled = True
                                    oItem = oForm.Items.Item("1000011")
                                    oItem.Enabled = True
                                Else
                                    oItem = oForm.Items.Item("1000008")
                                    oItem.Enabled = False
    ...
    Private Sub DOReport()
        Try
            ...
            cryRpt.Load(sPath & "\GK_FM\DO_WHMS_PR.rpt")
    ...
    Private Sub LoadDeliveryOrder(ByVal oform As SAPbouiCOM.Form)
        Try
            ...
            'oCombo = oMatrix.Columns.Item("V_7").Cells.Item(1).Specific
            'ComboLoad_Unit(oform, oCombo)
            'oform1.DataSources.UserDataSources.Add("V_0", SAPbouiCOM.BoDataType.dt_SHORT_TEXT)
            'oMatrix1 = oform1.Items.Item("1000001").Specific
            'oColumns = oMatrix1.Columns
            'oColumn = oColumns.Item("V_0")
            'oColumn.DataBind.SetBound(True, "", "V_0")
            'oItem = oform1.Items.Item("1000001")
            'oItem.Width = 150
            'oItem.Height = 30
            'oColumn.Width = 130
            'oMatrix1.AddRow()
    ...
    Private Sub LoadPaymentVoucher(ByVal oform As SAPbouiCOM.Form)
        Try
            ...
            oPV.PV_Bind(oform1, SBO_Application, "PR", Ocompany)
    ...
    Public Sub LoadDraftInvoice(ByVal JobNo As String)
        Try
            'Exit Sub
            oRecordSet1 = Ocompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRecordSet1.DoQuery("SELECT cast(T0.U_InterRem as Varchar(253)) as U_InterRem,T0.[U_QNo], T0.[U_CCode], T0.[U_CName], T0.[U_Atten], T0.[U_ConNo], T0.[U_JobNo], T0.[U_RefJob], T0.[U_LoadPortNC], T0.[U_DisPortNC], T0.[U_TotPkg], T0.[U_ChrgWt], T0.[U_VessVoyage], T0.[U_OBL], T0.[U_HBL], T0.[U_F1],T0.[U_GKBNo],T0.[U_VessName],T0.U_GrssWt,T0.[U_GKBNo],T0.[U_ItemDesc], T0.[U_ETA], T0.[U_ETD] FROM [dbo].[@AB_PRO_HEADER]  T0 WHERE T0.[U_JobNo]   ='" & JobNo & "'")
            If oRecordSet1.RecordCount = 0 Then
                SBO_Application.StatusBar.SetText("No Data Found!", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Exit Sub
            End If
            ...
            Try
                oEdit = oform1.Items.Item("16").Specific
                oEdit.String = oRecordSet1.Fields.Item("U_InterRem").Value
            Catch ex As Exception
            ...
            oCombo.Select("PR", SAPbouiCOM.BoSearchKey.psk_ByValue)
            ...
            For i = 1 To oRecordSet.RecordCount
                SBO_Application.StatusBar.SetText("Please Waite Data is Loading.Line No-" & i & "", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                oMatrix2.AddRow()
    ...
    Public Sub Payment_Voucher()
        Try
            ...
            sPath = IO.Directory.GetParent(Application.StartupPath).ToString
            'MsgBox(System.Windows.Forms.Application.StartupPath.ToString)
            cryRpt.Load(sPath & "\GK_FM\AB_RP005_PV_PR.rpt")
    ...
    Private Sub SBO_Application_MenuEvent(ByRef pVal As SAPbouiCOM.MenuEvent, ByRef BubbleEvent As Boolean) Handles SBO_Application.MenuEvent
        Try
            If pVal.BeforeAction = False And pVal.MenuUID = "1282" Then

                oForm = SBO_Application.Forms.ActiveForm
                If oForm.UniqueID = "PRO_JOB" Then
                    DocNumber_Local()
                    oEdit = oForm.Items.Item("SIJ18").Specific
                    oEdit.String = Format(Now.Date, "dd/MM/yy")
                End If
